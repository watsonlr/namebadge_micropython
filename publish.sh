#!/usr/bin/env bash
# publish.sh — copy all flash binaries to the org Pages repo,
# update manifest.json in the multi-binary format, and push.
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
MICROPYTHON_BUILD="${SCRIPT_DIR}/micropython_build"
APP_BIN="${MICROPYTHON_BUILD}/micropython.bin"
BL_BIN="${MICROPYTHON_BUILD}/bootloader/bootloader.bin"
PT_BIN="${MICROPYTHON_BUILD}/partition_table/partition-table.bin"
...
APP_NAME="MicroPython"
APP_DEST_NAME="micropython.bin"
BL_DEST_NAME="micropython_bl.bin"
PT_DEST_NAME="micropython_pt.bin"
...
# MicroPython uses an OTA partition table (factory at 0x20000).
# clearOtadata tells the flasher to wipe the OTA boot selector so the
# factory partition boots instead of whichever OTA slot was last active.
new_entry = {
    "name": "${APP_NAME}",
    "successMessage": "MicroPython flashed successfully!<br><small>MicroPython REPL is now running.<br>Connect via USB Serial (115200 baud) to start coding.</small>",
    "binaries": [
...
git add apps/
git commit -m "Update MicroPython ($(date '+%Y-%m-%d %H:%M'))
